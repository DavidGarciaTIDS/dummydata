...
const genAlumnosBtn = document.getElementById("genAlumnos");
const genSQLBtn = document.getElementById("genSQL");
const genCSVBtn = document.getElementById("genCSV");
const exportBtn = document.getElementById("export");

var salida = "";
var infoTabla = "";

genSQLBtn.disabled = true;
genCSVBtn.disabled = true;
exportBtn.disabled = true;

function generar_alumnos() {
  genAlumnosBtn.disabled = true;
...
'66${("0123456789" + i).slice(-8)}')${a}<br>`;
  ...
  genSQLBtn.disabled = false;
  genCSVBtn.disabled = false;
...
function generarSql() {
  if (salida != "") {
    ...
    generar.setAttribute(
      "href",
      "data:text/plain;charset=utf-8," + encodeURIComponent(contenidoSinBR)
    );
...
function generarCSV() {
  if (salida != "") {
    let caracteres = "'();`";
    let sinCaracteres = salida.replace(
      new RegExp("[" + caracteres + "]", "g"),
      ""
    );
